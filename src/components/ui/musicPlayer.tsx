...
const MusicPlayer: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!audioElement && songs[currentSong]) {
            ...
            audioElementRefs.current[currentSong]?.addEventListener("ended", switchToNextSong);
        ...
        if ("mediaSession" in navigator && songs[currentSong]) {
            ...
            navigator.mediaSession.setActionHandler("previoustrack", () => {
                ...
                if (audioElementRefs.current[currentSong] && audioElementRefs.current[currentSong]!.currentTime > 10) {
                    ...
                    const prevSongIndex = currentSong === 0 ? songs.length - 1 : currentSong - 1;
                    audioElementRefs.current[currentSong]!.currentTime = 0;
    ...
    const play = () => {
        setIsPlaying(true);
        audioElementRefs.current[currentSong]?.play().catch((error) => console.error(error));
    };
        ...
        <div className={styles.musicPlayer}>
            ...
            {isLoaded && (
                <div className={styles.controls}>
                    ...
                    <button className={styles.controlButton} onClick={isPlaying ? pause : play} id="music-button">
                        <FontAwesomeIcon icon={isPlaying ? "pause-circle" : "play-circle"} />
                    </button>
                    <button
                        className={styles.controlButton}
                        onClick={() => {
